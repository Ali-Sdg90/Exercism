...
describe("Random key cipher", () => {
    ...
    test("can encode", () => {
        ...
        expect(cipher.encode("aaaaaaaaaa")).toEqual(
            cipher.key.substring(0, 10)
        );
    });
    xtest("can decode", () => {
        expect(cipher.decode(cipher.key.substring(0, 10))).toEqual(
            "aaaaaaaaaa"
        );
    });
    xtest("is reversible", () => {
...
describe("Substitution cipher", () => {
    ...
    xtest("is reversible", () => {
        // I.e., if you apply decode in a encoded result, you must see
        // the same plaintext encode parameter as a result of the decode method
        expect(cipher.decode(cipher.encode("abcdefghij"))).toEqual(
            "abcdefghij"
        );
    });
    ...
    xtest("can encode messages longer than the key", () => {
        expect(new Cipher("abc").encode("iamapandabear")).toEqual(
            "iboaqcnecbfcr"
        );
    });
    xtest("can decode messages longer than the key", () => {
        expect(new Cipher("abc").decode("iboaqcnecbfcr")).toEqual(
            "iamapandabear"
        );
    });
});
